...
def send_failure_notification(object_key, error_message):
    try:
        ...
        print(
            f"Failure notification sent to SNS successfully for {object_key}: {error_message}"
        )
    except Exception as e:
        print(f"Error sending SNS failure notification for {object_key}: {e}")
...
def add_watermark(image, watermarkText):
    try:
        ...
        font_path = BASE_DIR / "fonts" / "Inter_28pt-ExtraBold.ttf"
        ...
        raise Exception(f"Error while adding watermark to image: {e}")
...
def save_image_to_s3(watermarked_img, object_key, img_format, bucket_name, s3_object):
    try:
        ...
        print(f"Watermarked image saved as '{new_s3_key}' in bucket '{bucket_name}'")
    except Exception as e:
        raise Exception(f"Error while saving processed image to S3: {e}")
...
def process_s3_image(bucket_name, object_key):
    try:
        ...
        watermark_text = head_response.get("Metadata", {}).get(
            "watermark-text", "Watermarky"
        )
        ...
        with Image.open(BytesIO(image_data)) as img:
            ...
            save_image_to_s3(
                watermarked_img, object_key, img.format, bucket_name, s3_object
            )

    except Exception as e:
        print(f"Error while processing image {object_key}: {e}")
        send_failure_notification(
            object_key, f"Error while processing image {object_key}: {e}"
        )
